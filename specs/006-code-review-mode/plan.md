...
supabase/migrations/
└── 0017_code_review_type.sql                         # drop + recreate questions_type_chk
...
| New migration on the `questions_type_chk` constraint | The constraint hardcodes the type list; adding `'code-review'` requires `drop + recreate`. Captured in 0017. |
